...
SIFT and REVEL scores were retreived using ANNOVAR with dbnsfp30a [@wang2010annovar,@liu2016dbnsfp] and from https://sites.google.com/site/revelgenomics/downloads [@ioannidis2016revel] respectively.
...
par(mfrow=c(2,2))
hist(protein_genes_w_paralogues_wide$number_of_paralogues, breaks = seq(0,50,1), xlab = "number of paralogues", ylab = "number of genes", main="A) Distribution of genes with paralogues", cex.main = 0.7)
...
Total = c(length(p.paralogous_var_align$paralog_data$ID),length(b.paralogous_var_align$paralog_data$ID),"NA","NA","NA","NA")
Paralogue_Annotation_no_QC = c(format(conf$con_table[2], scientific = FALSE),format(conf$con_table[4], scientific = FALSE),round(conf$PPV, digits = 3),round(conf$Sensitivity, digits = 3),round(conf$Specificity, digits = 3),round(conf$Pvalue$p.value, digits = 3))
Variants_remaining_after_PA_QC1 = c(format(conf2$con_table[2], scientific = FALSE),format(conf2$con_table[4], scientific = FALSE),round(conf2$PPV, digits = 3), round(conf2$Sensitivity, digits = 3),round(conf2$Specificity, digits = 3), conf2$Pvalue$p.value)
Variants_removed_after_PA_QC1 = c(var_removed$con_table[2],var_removed$con_table[4],round(var_removed$PPV, digits = 3),"NA","NA",round(var_removed$Pvalue$p.value, digits = 3))
Variants_remaining_after_PA_QC2 = c(format(conf3$con_table[2], scientific = FALSE),format(conf3$con_table[4], scientific = FALSE),round(conf3$PPV, digits = 3),round(conf3$Sensitivity, digits = 3),round(conf3$Specificity, digits = 3),round(conf3$Pvalue$p.value, digits = 3))
Variants_removed_after_PA_QC2 = c(var_removed2$con_table[2],var_removed2$con_table[4],round(var_removed2$PPV, digits = 3),"NA","NA",round(var_removed2$Pvalue$p.value, digits = 3))
Variants_remaining_after_PA_QC3 = c(format(conf4$con_table[2], scientific = FALSE),format(conf4$con_table[4], scientific = FALSE),round(conf4$PPV, digits = 3),round(conf4$Sensitivity, digits = 3),round(conf4$Specificity, digits = 3),round(conf4$Pvalue$p.value, digits = 3))
Variants_removed_after_PA_QC3 = c(var_removed3$con_table[2],var_removed3$con_table[4],round(var_removed3$PPV, digits = 3),"NA","NA",round(var_removed3$Pvalue$p.value, digits = 3))
...
rownames(clinvar_set) = c("Total variants", "Paralogue Annotation", "Variants remaining after QC1", "Variants removed from QC1", "Variants remaining after QC2", "Variants removed after QC2", "Variants remaining after QC3", "Variants removed after QC3")
kable(clinvar_set)
# pander(clinvar_set2)
# pander(clinvar_set3)
...
PA_threshold_row = c(format(paraz.conf$con_table[2], scientific = FALSE), format(paraz.conf$con_table[2,2], scientific = FALSE), round(paraz.conf$PPV, digits = 3), round(paraz.conf$Sensitivity, digits = 3), round(paraz.conf$Specificity,digits = 3), round(paraz.conf$Pvalue$p.value, digits = 3))
...
ggplot(prc_data, aes(x=Sensitivity, y=Precision, colour = cutoff)) + geom_point() + scale_color_gradient(low="blue", high="red", limits=c(0, 11))
...
Paralogue_Annotation_no_QC = c(format(conf$con_table[2], scientific = FALSE),format(conf$con_table[4], scientific = FALSE),round(digits = 3, conf$PPV),round(digits = 3,conf$Sensitivity),round(digits = 3,conf$Pvalue$p.value))
Variants_remaining_after_PA_QC1 = c(format(conf2$con_table[2], scientific = FALSE),format(conf2$con_table[4], scientific = FALSE),round(digits = 3,conf2$PPV), round(digits = 3,conf2$Sensitivity), round(digits = 3,conf2$Pvalue$p.value))

Variants_removed_after_PA_QC1 = c(var_removed$con_table[2],var_removed$con_table[4],round(digits = 3,var_removed$PPV),"NA",round(digits = 3,var_removed$Pvalue$p.value))
Variants_remaining_after_PA_QC2 = c(format(conf3$con_table[2], scientific = FALSE),format(conf3$con_table[4], scientific = FALSE),round(digits = 3,conf3$PPV),round(digits = 3,conf3$Sensitivity),round(digits = 3,conf3$Pvalue$p.value))
Variants_removed_after_PA_QC2 = c(var_removed2$con_table[2],var_removed2$con_table[4],round(digits = 3,var_removed2$PPV),"NA",round(digits = 3,var_removed2$Pvalue$p.value))
Variants_remaining_after_PA_QC3 = c(format(conf4$con_table[2], scientific = FALSE),format(conf4$con_table[4], scientific = FALSE),round(digits = 3,conf4$PPV),round(digits = 3,conf4$Sensitivity),round(digits = 3,conf4$Pvalue$p.value))
Variants_removed_after_PA_QC3 = c(var_removed3$con_table[2],var_removed3$con_table[4],round(digits = 3,var_removed3$PPV),"NA",round(digits = 3,var_removed3$Pvalue$p.value))
...
##control data VEP##
control.paralogous_var_align = Paralogous_var_align("data/case_and_control/remapped_ExAC_missense_sarcomeric_chr_wRefAl.out_paraloc_paralogs2.noQC", "data/case_and_control/remapped_ExAC_missense_sarcomeric_chr_wRefAl.out_paraloc_tableized", "data/clinvar/clinvar_alleles.single.b38.Pathogenic_and_LikelyPathogenic.out_paraloc_tableized") #only rare variants in the control cohort
...
# for (gene in c("MYBPC3")){
  ...
  tmp_df = data.frame(gene,all_rare_vars$OR,all_rare_vars$EF,pa_predicted_rare_vars$OR,pa_predicted_rare_vars$EF)
  gene_EFs = rbind(gene_EFs,tmp_df)
}
##Overall##
...
gene_EFs = rbind(gene_EFs,data.frame(gene="Overall",all_rare_vars$OR,all_rare_vars$EF,pa_predicted_rare_vars$OR,pa_predicted_rare_vars$EF))
...
\newpage
## Comparison to SIFT/REVEL
```{r cf_sift_revel, cache=TRUE, echo = FALSE, results='asis', message = FALSE, warning=FALSE}
p.normal_PA = Paralogous_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs2.noQC", "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized")

##HAVE A THINK ABOUT THE CODE BELOW. DO I REALLY NEED TO IT THIS WAY? MAKE IT SIMPLER. I'M JUST COMPARING PA WITH SIFT/REVEL FFS....

##Preparing sift and revel files
#Pathogenic variants

# Sift_tableized_file = read.csv(file="data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs_no_empty_lines.hg19_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE)
...
# Sift_tableized_data = Sift_tableized_file[!is.na(Sift_tableized_file$SIFT_score),]
...
# Revel_tableized_data = Revel_tableized_file[!is.na(Revel_tableized_file$Amino_acids)&!is.na(Revel_tableized_file$REVEL_score),]
...
Revel_tableized_data$REF_Amino_acids = sapply(Revel_tableized_data[,"Amino_acids"],strsplit, "/")
Revel_tableized_data$REF_Amino_acids = sapply(Revel_tableized_data[,"REF_Amino_acids"],function(x) x[1])
Revel_tableized_data$ALT_Amino_acids = sapply(Revel_tableized_data[,"Amino_acids"],strsplit, "/")
Revel_tableized_data$ALT_Amino_acids = sapply(Revel_tableized_data[,"ALT_Amino_acids"],function(x) x[2])

sift_revel_data = right_join(Revel_tableized_data,Sift_tableized_data, by =  c("Variant_pos", "ID", "REF", "ALT"))
# sift_revel_data = sift_revel_data[!is.na(sift_revel_data$REVEL_score)&!is.na(sift_revel_data$SIFT_score),]
sift_revel_data = sift_revel_data[!duplicated(sift_revel_data),]

#Benign variants


# b.Sift_tableized_file = read.csv(file="data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_paralogs_no_empty_lines.hg19_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE)
...
b.Sift_tableized_data = b.Sift_tableized_file[!is.na(b.Sift_tableized_file$SIFT_score),]
...
b.Revel_tableized_data = b.Revel_tableized_file[!is.na(b.Revel_tableized_file$Amino_acids)&!is.na(b.Revel_tableized_file$REVEL_score),]
b.Revel_tableized_data$Variant_pos = paste(b.Revel_tableized_data$CHROM,b.Revel_tableized_data$POS, sep = " ")
b.Revel_tableized_data$REF_Amino_acids = sapply(b.Revel_tableized_data[,"Amino_acids"],strsplit, "/")
b.Revel_tableized_data$REF_Amino_acids = sapply(b.Revel_tableized_data[,"REF_Amino_acids"],function(x) x[1])
b.Revel_tableized_data$ALT_Amino_acids = sapply(b.Revel_tableized_data[,"Amino_acids"],strsplit, "/")
b.Revel_tableized_data$ALT_Amino_acids = sapply(b.Revel_tableized_data[,"ALT_Amino_acids"],function(x) x[2])

b.sift_revel_data = right_join(b.Revel_tableized_data,b.Sift_tableized_data, by =  c("Variant_pos", "ID", "REF", "ALT"))
b.sift_revel_data = b.sift_revel_data[!is.na(b.sift_revel_data$REVEL_score)&!is.na(b.sift_revel_data$SIFT_score),]
b.sift_revel_data = b.sift_revel_data[!duplicated(b.sift_revel_data),]

##Predicting pathogenic
p.Sift_Revel_var_align = Sift_Revel_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs2.noQC", "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized", sift_revel_data)
# p.Sift_Revel_var_align$Total_paralog_annotations = p.Sift_Revel_var_align$Total_paralog_annotations[!is.na(p.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score)&!is.na(p.Sift_Revel_var_align$Total_paralog_annotations$SIFT_score),]
p.Total_number_of_vars = length(p.Sift_Revel_var_align$paralog_data$ID)
p.num_of_paralog_anno = sum(!is.na(p.Sift_Revel_var_align$Total_paralog_annotations$ID.y))
p.num_of_sift = length(which(p.Sift_Revel_var_align$Total_paralog_annotations$SIFT_pred == "D"))
p.num_of_revel = length(which(p.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score > 0.5))

btop.Sift_Revel_var_align = Sift_Revel_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_paralogs2.all_con", b.sift_revel_data, "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized")
btop.Sift_Revel_var_align$Total_paralog_annotations = btop.Sift_Revel_var_align$Total_paralog_annotations[!is.na(btop.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score)&!is.na(btop.Sift_Revel_var_align$Total_paralog_annotations$SIFT_score),]
btop.num_of_paralog_anno = sum(!is.na(btop.Sift_Revel_var_align$Total_paralog_annotations$ID.y))
btop.num_of_sift = length(which(btop.Sift_Revel_var_align$Total_paralog_annotations$SIFT_pred == "D"))
btop.num_of_revel = length(which(btop.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score > 0.5))

PA_PPV = p.num_of_paralog_anno/(p.num_of_paralog_anno+btop.num_of_paralog_anno)
sift_PPV = p.num_of_sift/(p.num_of_sift+btop.num_of_sift)
revel_PPV = p.num_of_revel/(p.num_of_revel+btop.num_of_revel)

##Predicting Benign
ptob.Sift_Revel_var_align = Sift_Revel_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs2.all_con", sift_revel_data, "data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_tableized")
ptob.Sift_Revel_var_align$Total_paralog_annotations = ptob.Sift_Revel_var_align$Total_paralog_annotations[!is.na(ptob.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score)&!is.na(ptob.Sift_Revel_var_align$Total_paralog_annotations$SIFT_score),]
ptob.num_of_paralog_anno = sum(!is.na(ptob.Sift_Revel_var_align$Total_paralog_annotations$ID.y))
ptob.num_of_sift = length(which(ptob.Sift_Revel_var_align$Total_paralog_annotations$SIFT_pred == "T"))
ptob.num_of_revel = length(which(ptob.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score <= 0.5))


b.Sift_Revel_var_align = Sift_Revel_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_paralogs2.noQC", b.sift_revel_data, "data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_tableized")
b.Sift_Revel_var_align$Total_paralog_annotations = b.Sift_Revel_var_align$Total_paralog_annotations[!is.na(b.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score)&!is.na(b.Sift_Revel_var_align$Total_paralog_annotations$SIFT_score),]
b.Total_number_of_vars = length(b.Sift_Revel_var_align$paralog_data$ID)
b.num_of_paralog_anno = sum(!is.na(b.Sift_Revel_var_align$Total_paralog_annotations$ID.y))
b.num_of_sift = length(which(b.Sift_Revel_var_align$Total_paralog_annotations$SIFT_pred == "T"))
b.num_of_revel = length(which(b.Sift_Revel_var_align$Total_paralog_annotations$REVEL_score <= 0.5))

Sum_table = data.frame(c(p.Total_number_of_vars,b.Total_number_of_vars,"NA"),
                       c(format(p.num_of_paralog_anno, scientific = FALSE),format(btop.num_of_paralog_anno,scientific = FALSE),round(PA_PPV,digits=3)),
                       c(format(p.num_of_sift, scientific = FALSE),format(btop.num_of_sift, scientific = FALSE),round(sift_PPV, digits = 3)),
                       c(format(p.num_of_revel, scientific = FALSE), format(btop.num_of_revel, scientific = FALSE), round(revel_PPV, digits = 3))
                       )
rownames(Sum_table) = c("Pathogenic", "Benign","PPV")                       
colnames(Sum_table) = c("Total", "Pathogenic variants predicted by PA", "Pathogenic variants predicted by SIFT", "Pathogenic variants predicted by REVEL")


x = p.normal_PA$paralog_data[,c("Variant_pos","ID")]
x2 = ddply(x,.(Variant_pos,ID),nrow)
x2$Coder = "X"
y = p.Sift_Revel_var_align$paralog_data[,c("Variant_pos", "ID")]
y2 = ddply(y,.(Variant_pos,ID),nrow)
y2$Coder = "Y"
z = rbind(x2, y2)
dupRows <- dupsBetweenGroups(z, "Coder")
z2 = cbind(z, unique=dupRows)

```

NOTE: BELOW NOT GOING TO WORK. NEED TO TAKE SAME VARIANTS THAT WERE IN GENES WITH PARALOGUES IF CF WITH OTHER TOOLS!!!!!

```{r cf_sift_revel, cache=TRUE, echo = FALSE, results='asis', message = FALSE, warning=FALSE}
# p.normal_PA = Paralogous_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs2.noQC", "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized")
# b.normal_PA = Paralogous_var_align("data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_paralogs2.noQC", "data/clinvar/clinvar_20181028_GRCh37_onlyBenign_and_Likely_benign.out_paraloc_tableized", "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized")
# conf = conf_matrix(p.normal_PA$num_of_paralog_anno, p.normal_PA$paralog_data, b.normal_PA$num_of_paralog_anno, b.normal_PA$paralog_data)
...
# conf2 = conf_matrix(p.normal_PA2$num_of_paralog_anno, p.normal_PA2$paralog_data, b.normal_PA2$num_of_paralog_anno, b.normal_PA2$paralog_data)
...
Revel_tableized_data = filter(Revel_tableized_data, ID %in% p.normal_PA2$n_occur$Var1)
...
b.Revel_tableized_data = filter(b.Revel_tableized_data, ID %in% b.normal_PA2$n_occur$Var1)
...
#PA
p.normal_PA2 = Subset_var_align_id(Revel_tableized_data$ID,"data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_paralogs2.all_con", "data/clinvar/clinvar_20181028_GRCh37_onlyPathogenic_and_Likely_pathogenic.out_paraloc_tableized")
