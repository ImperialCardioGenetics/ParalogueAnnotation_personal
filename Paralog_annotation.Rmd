...
patho_art_var = dplyr::filter(p.art.paralogous_var_align$paralog_data, Gene %in% gene_subset)  
patho_art_var = p.art.paralogous_var_align$Total_paralog_annotations[!is.na(p.art.paralogous_var_align$Total_paralog_annotations$ID.y),]

```


But querying every possible missense variant at all positions in the 8 sarcomeric genes can be done (` r length(p.art.paralogous_var_align$paralog_data$Variant_pos)` total variants). Doing so, predicts `r p.art.paralogous_var_align$num_of_paralog_anno` variants to pathogenic - a `r p.art.paralogous_var_align$num_of_paralog_anno/p.sarco_var_align$num_of_paralog_anno` fold increase. 
