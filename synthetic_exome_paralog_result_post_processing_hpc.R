...
for (j in 1:22){
        for (k in c("noQC","para_con","all_con")){
                ...
                write.table(Total_annotations,file=paste0("/work/nyl112/data/synthetic_exome/Total_annotations_chrom_",j,"_",k,"_predicted_pathogenic.vcf"), na="", row.names = FALSE, col.names = TRUE, sep = ",", quote=FALSE)
        }
}
...
for (k in c("noQC","para_con","all_con")){
        ...
        save(Total_annotations, file = paste0("/work/nyl112/data/synthetic_exome/Total_annotations_data_chrom_X_",k,".RData"))
        Total_annotations = Total_annotations[,c("CHROM.x","POS.x","ID.x","REF.x","ALT.x")]
        names(Total_annotations) = c("CHROM","POS","ID","REF","ALT")
        write.table(Total_annotations,file=paste0("/work/nyl112/data/synthetic_exome/Total_annotations_chrom_X_",k,"_predicted_pathogenic.vcf"), na="", row.names = FALSE, col.names = TRUE, sep = ",", quote=FALSE)
}
