...
# sift_conf_test = conf_matrix(p.num_of_sift,other_tools_file,b.num_of_sift,b.other_tools_file)
...
other_tools_set = t(data.frame(
  ...
  Cadd = c(sapply(c(cadd_conf$con_table[2],cadd_conf$con_table[4]),format,scientific = FALSE),
           sapply(c(cadd_conf$PPV,cadd_conf$Sensitivity,cadd_conf$Specificity,cadd_conf$Pvalue$p.value),signif,digits=3)),
...
other_tools_data = data.frame(
  ...
  Sensitivity = c(as.numeric(other_tools_set[2:9,4]))
  )
...
gg4 = ggplot(other_tools_data, aes(x=Sensitivity, y=Precision, colour = Type, shape = Type)) + geom_point(size=5) + scale_shape_manual(values=c(0, 1, 2, 5, 15, 16, 17, 18)) +
...
other_tools = read.csv(file="data/TP53-TA/4704961/TP53-TA_deleterious.vcf.hg38_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE) 
...
b.other_tools = read.csv(file="data/TP53-TA/4704961/TP53-TA_benign.vcf.hg38_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE) 
...
other_tools = read.csv(file="data/UniFun/4704955/UniFun_deleterious.vcf.hg38_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE) 
p.num_of_revel = length(which(other_tools$REVEL_score > 0.5))
p.num_of_sift = length(which(other_tools$SIFT_pred == "D"))
p.num_of_mcap = length(which(other_tools$M.CAP_pred == "D"))
p.num_of_cadd = length(which(other_tools$CADD_phred > 15))

b.other_tools = read.csv(file="data/UniFun/4704955/UniFun_benign.vcf.hg38_multianno.vcf_tableized_org", sep = "\t", header=TRUE, stringsAsFactors=FALSE) 
b.num_of_revel = length(which(b.other_tools$REVEL_score > 0.5))
b.num_of_sift = length(which(b.other_tools$SIFT_pred == "D"))
b.num_of_mcap = length(which(b.other_tools$M.CAP_pred == "D"))
b.num_of_cadd = length(which(b.other_tools$CADD_phred > 15))
