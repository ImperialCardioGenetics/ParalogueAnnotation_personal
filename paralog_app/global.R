...
predict_output<-function(output,input_data){
  ...
  raw_data<-subset(raw_data,select=c(var, Gene, CHROM.y ,POS.y,REF.y,ALT.y,ID.y ,SYMBOL,Protein_position.y,
                                     REF_Amino_acids.y,ALT_Amino_acids.y ,Codons.y,Para_Z_score.y),)
  # rename dataframe columns for webpage
  colnames(raw_data)<-c("Variant_ID","Query_Gene","Chr","Position","REF","ALT","ClinVar_ID","Gene","Protein Position","Reference AA", "Alt AA","Codons","para_Z_score" )
