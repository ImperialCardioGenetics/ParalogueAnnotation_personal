...
But querying every possible missense variant at all positions in the 8 sarcomeric genes can be done (` r length(p.art_paralog_data$Variant_pos)` total variants). Doing so, predicts `r length(p.art_num_of_paralog_anno$Variant_pos)` variants to pathogenic - a `r length(p.art_num_of_paralog_anno$Variant_pos)/p.sarco_var_align$num_of_paralog_anno` fold increase. Though how many additional predictions are true positives would require validation.

However, it must be noted that genes with few predictions should not discourage use of PA...
