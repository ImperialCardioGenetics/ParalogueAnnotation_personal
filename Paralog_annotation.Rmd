...
art.paralogous_var_align = Paralogous_var_align_compressed("data/all_possible_mutations/cm_mutation_vep_canon.out_paraloc_paralogs2.noQC.gz", "data/all_possible_mutations/cm_mutation_vep_canon.out_paraloc_tableized.gz", "data/clinvar/clinvar_alleles.single.b38.Pathogenic_and_LikelyPathogenic.out_paraloc_tableized")
...
According to ensembl, `r length(protein_genes_w_paralogues_genes)` protein coding genes are defined to have paralogues. While `r length(protein_genes_wo_paralogues_genes)` protein coding genes do not have paralogues.
In the clinvar pathogenic and likely pathogenic dataset, there's `r length(clinvar_P_LP_tableized$CHROM)` variants from `r length(clinvar_P_LP_tableized_genes)` genes. `r length(clinvar_P_LP_genes_wo_paralogs)` of these genes do not have paralogs and therefore the `r length(clinvar_P_LP_variants_wo_paralogs)` variants lying within these genes were not used for annotation.
